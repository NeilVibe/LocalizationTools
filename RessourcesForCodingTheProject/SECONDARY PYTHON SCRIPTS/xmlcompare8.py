...
class TranslationComparer:
    ...
    def start_export_differences_xml(self):
        if len(self.source_folders)!=1 or len(self.target_folders)!=1:
            messagebox.showerror("Error","Select exactly one source and one target for XML export.")
            return
        default="EN_temp_translations"
        fname = simpledialog.askstring("Name","Output XML name (no .xml):",initialvalue=default)
        if not fname: return
        save = filedialog.asksaveasfilename(defaultextension=".xml",
                    filetypes=[("XML","*.xml")], initialfile=f"{fname}.xml")
        if not save: return
        threading.Thread(target=self.export_differences_xml,
                         args=(self.source_folders[0],
                               self.target_folders[0],
                               fname, save),
                         daemon=True).start()

    def export_differences_xml(self, src, tgt, fname, save_path):
        ...
        self.root.update()
        ssub,_ = self.find_subset_files(src)
        tsub,_ = self.find_subset_files(tgt)
        stbl,_,_,_ = self.build_mega_table(src,ssub)
        ttbl,_,_,_ = self.build_mega_table(tgt,tsub)
        ssigs=set(stbl); tsigs=set(ttbl)
        miss = ssigs-tsigs
        all_missing=[]
        for sig in miss:
            for occ in stbl[sig]:
                all_missing.append(occ["element"])
        root = etree.Element("root", FileName=f"{fname}.xml")
        total=len(all_missing)
        for i,loc in enumerate(all_missing):
            xmlb = ET.tostring(loc,encoding="utf-8")
            el = etree.fromstring(xmlb)
            root.append(el)
            if (i+1)%100==0 or i+1==total:
                self.progress['value']=(i+1)/total*100
                self.root.update()
        try:
            xb = etree.tostring(root,pretty_print=True,encoding="UTF-8",xml_declaration=False)
            with open(save_path,"wb") as f: f.write(xb)
            self.output_text.insert(tk.END,f"Exported {total} rows →{save_path}\n")
            self.progress_label.config(text="Export done")
            messagebox.showinfo("Done",f"Exported {total} rows to:\n{save_path}")
        except Exception as e:
