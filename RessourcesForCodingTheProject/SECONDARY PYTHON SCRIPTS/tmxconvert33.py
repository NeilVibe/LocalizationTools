...
def postprocess_tmx_string(xml_str):
    ...
    # -------------------------------------------------------
    # Pattern  {Staticinfo:Knowledge:IDENT#INNER}
    sk_pattern = re.compile(r'\{Staticinfo:Knowledge:([^#}]+)#([^}]+)\}')

    # We process *each* <seg> individually so the i='n' counter resets per segment
...
def combine_xmls_to_tmx(input_folder, output_file, target_language, postprocess=True):
    ...
    • Main strings: keep only newest duplicate (KR + StringId).
    • Description strings: NO de-duplication – every TU is kept.
    """
    try:
        ...
        main_tu_map = {}  # KR + StringId  → (mtime, tu)
        ...
        for idx, xml_file_path in enumerate(xml_files, 1):
            ...
            for loc in xml_root.iter("LocStr"):
                ...
                # MAIN TU (deduplicated)
                if korean_text and target_text and not is_korean(target_text):
                    tu = etree.Element("tu", creationid="CombinedConversion", changeid="CombinedConversion")
                    tuv_ko = etree.SubElement(tu, "tuv", **{f"{{{XML_NS}}}lang": "ko"})
                    etree.SubElement(tuv_ko, "seg").text = korean_text
                    ...
                    etree.SubElement(tu, "prop", type="x-context").text = string_id or "NoStringId"

                    key = (korean_text, string_id)
                    if key not in main_tu_map or file_mtime > main_tu_map[key][0]:
                        main_tu_map[key] = (file_mtime, tu)

                # DESCRIPTION TU (no de-duplication any more)
                if string_id and desc_origin and desc_text and not is_korean(desc_text):
                    if desc_origin.strip() in ("&desc;", "&amp;desc;") and desc_text.strip() in ("&desc;", "&amp;desc;"):
                        continue
                    if not desc_origin.startswith("&desc;") and not desc_origin.startswith("&amp;desc;"):
                        desc_origin = "&desc;" + desc_origin

                    desc_tu = etree.Element("tu", creationid="PAAT", changeid="PAAT")
                    ...
                    desc_tu_list.append(desc_tu)   # ← keep every description TU
