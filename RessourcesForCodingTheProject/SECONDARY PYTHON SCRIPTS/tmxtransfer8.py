...
status_label         = None
info_panel           = None
mode_var             = None  # "folder" or "file"

root = None  # will be set in main()

# ──────────────────────────────────────────────────────────────────────
#  PLACEHOLDER POST-PROCESSING  (COPIED FROM MAIN TOOL)
# ──────────────────────────────────────────────────────────────────────
def postprocess_tmx_string(xml_str):
    ...
    # 1) Staticinfo:Knowledge → <bpt>/<ept>
    ...
    # 2) Generic placeholders → <ph>
    xml_str = replace_outside_ph(
        r'%(\d)#',
        lambda m: f'<ph>&lt;mq:rxt-req displaytext="Param{m.group(1)}" '
                  f'val="%{m.group(1)}#" /&gt;</ph>',
        xml_str)

    xml_str = replace_outside_ph(
        r'\{(?!Staticinfo:Knowledge:)([^}]+)\}',
        lambda m: f'<ph>&lt;mq:rxt-req displaytext="{m.group(1)}" '
                  f'val="{{{m.group(1)}}}" /&gt;</ph>',
        xml_str)

    xml_str = replace_outside_ph(
        r'\\(\w)',
        lambda m: f'<ph>&lt;mq:rxt displaytext="\\{m.group(1)}" '
                  f'val="\\{m.group(1)}" /&gt;</ph>',
        xml_str)
...
# ──────────────────────────────────────────────────────────────────────
#  TMX PRE-PROCESS (for reading TMX)
# ──────────────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────────────
#  PARSE TMX  (reading input TMX for matching)
# ──────────────────────────────────────────────────────────────────────
def parse_tmx(tmx_path):
    ...
    print(f"[TMX]   normal units: {len(translation_units)}   "
          f"description units: {len(desc_units)}")
...
def xml_to_excel():
    ...
    for path in xml_files:
        _, root_elem = robust_parse_xml(path)
        if root_elem is None:
            continue
        for loc in root_elem.iter("LocStr"):
            rows.append((
                (loc.get("StrOrigin",  "") or "").strip(),
                (loc.get("Str",        "") or "").strip(),
                (loc.get("StringId",   "") or "").strip(),
                (loc.get("DescOrigin", "") or "").strip(),
                (loc.get("Desc",       "") or "").strip()
            ))
...
def excel_to_tmx(is_memoq=False):
    ...
    for excel_file in excel_files:
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):
            ...
            normal_ok = bool(src and tgt and ctx and not is_korean(tgt))
            desc_ok   = bool(desc_src and desc_tgt and ctx and not is_korean(desc_tgt))

            if not (normal_ok or desc_ok):
                continue
    ...
    messagebox.showinfo("Success", f"Excel → {'MemoQ ' if is_memoq else ''}TMX saved:\n{out_path}")
...
def tmx_to_excel():
    ...
    XML_NS = "{http://www.w3.org/XML/1998/namespace}"
    MEMOQ_FLAG = '<mq:rxt-req displaytext="desc" val="&amp;desc;" />'

    def is_desc(txt):
        txt = txt.lstrip()
        return txt.startswith("&desc;") or txt.startswith("&amp;desc;") or txt.startswith(MEMOQ_FLAG[:10])

    def strip_desc(txt):
        txt = txt.lstrip()
        if txt.startswith("&desc;"):        return txt[6:].lstrip()
        if txt.startswith("&amp;desc;"):    return txt[10:].lstrip()
        if txt.startswith("<mq:rxt-req"):
            end = txt.find("/>")
            return txt[end+2:].lstrip() if end != -1 else txt
        return txt
    ...
    for tmx_path in tmx_files:
        with open(tmx_path, "r", encoding="utf-8") as f:
            raw = f.read()
        pre = preprocess_tmx_content(raw)
        pre = re.sub(r'^<\?xml[^>]*\?>\s*', '', pre, flags=re.MULTILINE)
        pre = re.sub(r'<!DOCTYPE[^>]*>\s*', '', pre, flags=re.MULTILINE)
        root_elem = etree.fromstring(pre.encode("utf-8"), parser=etree.XMLParser(recover=True))
        for tu in root_elem.findall(".//tu"):
            ctx = next((p.text.strip() for p in tu.findall("prop")
                        if p.attrib.get("type") == "x-context" and p.text), "NoStringId")
            ko_txt = en_txt = ""
            for tuv in tu.findall("tuv"):
                lang = (tuv.get(f"{XML_NS}lang") or tuv.get("xml:lang") or "").lower()
                seg = tuv.find("seg")
                if seg is None: continue
                rawtxt = seg.text.strip() if seg.text else ""
                if lang.startswith("ko"):
                    ko_txt = rawtxt
                elif lang.startswith("en"):
                    en_txt = rawtxt
            if not (ko_txt or en_txt): continue
            row = rows_by_ctx.get(ctx, ["", "", ctx, "", ""])
            if is_desc(ko_txt):
                row[3] = strip_desc(ko_txt)
                row[4] = strip_desc(en_txt)
            else:
                row[0] = ko_txt
                row[1] = en_txt
            rows_by_ctx[ctx] = row
    ...
    rows = sorted(rows_by_ctx.values(), key=lambda r: r[2])
    out_path = (filedialog.asksaveasfilename(defaultextension=".xlsx",
                                             filetypes=[("Excel Files", "*.xlsx")])
                if mode_var.get() == "folder"
                else os.path.splitext(tmx_files[0])[0] + ".xlsx")
    if not out_path: return
    wb = openpyxl.Workbook()
    ws = wb.active
    ws.append(["StrOrigin", "Str", "StringId", "DescOrigin", "DescText"])
...
# ──────────────────────────────────────────────────────────────────────
#  SIMPLE TRANSLATE (threaded, updated UI)
# ──────────────────────────────────────────────────────────────────────
def simple_translate_krid():
    if not tmx_file_path or not (xml_folder_path or xml_file_path):
        messagebox.showerror("Error", "Please select XML and TMX.")
        return
            ...
            for loc in root_elem.iter("LocStr"):
                ...
                if (sid, orig) in perfect_str:
                    ...
                    if new and new != old_s:
                        ...
                        updated_s += 1
                        changed = True
                if (sid, dorig) in perfect_desc:
                    new = perfect_desc[(sid, dorig)]
                    if new and new != old_d:
                        loc.set("Desc", new)
                        updated_d += 1
                        changed = True
            if changed:
                tree_obj.write(path, encoding="utf-8", xml_declaration=False, pretty_print=True)
...
# ──────────────────────────────────────────────────────────────────────
#  TWO-STEP TRANSLATE (threaded, updated UI)
# ──────────────────────────────────────────────────────────────────────
...
def generate_stringid_from_eventname_dialogvoice():
    ...
    wb.save(out); messagebox.showinfo("Done", f"String IDs saved:\n{out}")
