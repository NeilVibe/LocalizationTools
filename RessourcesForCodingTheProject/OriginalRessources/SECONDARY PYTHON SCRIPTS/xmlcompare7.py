...
class TranslationComparer:
    def __init__(self, root):
        ...
        # Check StrOrigin Changes   
        tk.Button(main_frame,
                  text="Check StrOrigin Changes",
                  command=self.start_check_strorigin_changes,
                  bg="#3F51B5", fg="white", font=("Arial", 12), pady=10).pack(pady=(0, 20))
    ...
    def check_translation_changes_between_source_target(self):
        self.progress_label.config(text="Checking for translation (Str) changes and additions...")
        self.progress['value'] = 0
        self.output_text.delete(1.0, tk.END)
        self.root.update()
        ...
        total_change_words = 0
        total_addition_chars = 0
        total_addition_words = 0
        file_change_counts = {}
        file_addition_counts = {}
        ...
        for file_id in sorted(common_files):
            ...
            src_values_set = set(src_dict.values())

            common_keys = set(src_dict.keys()) & set(tgt_dict.keys())
            new_keys = set(tgt_dict.keys()) - set(src_dict.keys())

            diffs = []
            additions = []

            for key in common_keys:
                if src_dict[key] != tgt_dict[key]:
                    diffs.append({
                        'StringId': key,
                        'SourceStr': src_dict[key],
                        'TargetStr': tgt_dict[key]
                    })
                    total_change_chars += len(tgt_dict[key])
                    total_change_words += len(tgt_dict[key].split())

            for key in new_keys:
                if tgt_dict[key] not in src_values_set:
                    additions.append({
                        'StringId': key,
                        'TargetStr': tgt_dict[key]
                    })
                    total_addition_chars += len(tgt_dict[key])
                    total_addition_words += len(tgt_dict[key].split())

            if diffs:
                total_changes += len(diffs)
                file_change_counts[file_id] = len(diffs)
                details_lines.append(f"File : \"{file_id}\"")
                details_lines.append(f"  CHANGES ({len(diffs)}):")
                for d in diffs[:10]:
                    details_lines.append(f"    - StringId=\"{d['StringId']}\"")
                    details_lines.append(f"        Source Str : \"{d['SourceStr']}\"")
                    details_lines.append(f"        Target Str : \"{d['TargetStr']}\"")
                if len(diffs) > 10:
                    details_lines.append(f"    ... and {len(diffs)-10} more changes")
                details_lines.append("")

            if additions:
                total_additions += len(additions)
                file_addition_counts[file_id] = len(additions)
                details_lines.append(f"File : \"{file_id}\"")
                details_lines.append(f"  ADDITIONS ({len(additions)}):")
                for a in additions[:10]:
                    details_lines.append(f"    - StringId=\"{a['StringId']}\"")
                    details_lines.append(f"        Target Str : \"{a['TargetStr']}\"")
                if len(additions) > 10:
                    details_lines.append(f"    ... and {len(additions)-10} more additions")
                details_lines.append("")

        report_lines = []
        report_lines.append("TRANSLATION (Str) CHANGES & ADDITIONS REPORT")
        report_lines.append("=" * 80)
        report_lines.append(f"Source folder: {source_folder}")
        report_lines.append(f"Target folder: {target_folder}")
        report_lines.append(f"Generated on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        report_lines.append("CHANGES = same StringId, different Str\nADDITIONS = new StringId with new Str value\n")

        report_lines.append("\nSUMMARY")
        report_lines.append("-" * 80)
        report_lines.append(f"Total files compared : {total_files}")
        report_lines.append(f"Total Str changes : {total_changes}")
        report_lines.append(f"Total changed characters (changes) : {total_change_chars}")
        report_lines.append(f"Total changed words (changes) : {total_change_words}")
        report_lines.append(f"Total Str additions : {total_additions}")
        report_lines.append(f"Total added characters (additions) : {total_addition_chars}")
        report_lines.append(f"Total added words (additions) : {total_addition_words}")
        report_lines.append(f"Grand total characters (changes+additions) : {total_change_chars + total_addition_chars}")
        report_lines.append(f"Grand total words (changes+additions) : {total_change_words + total_addition_words}")

        top_changes = sorted(file_change_counts.items(), key=lambda x: x[1], reverse=True)[:50]
        top_additions = sorted(file_addition_counts.items(), key=lambda x: x[1], reverse=True)[:50]

        report_lines.append("\nTOP 50 FILES WITH MOST CHANGES")
        for file_id, change_count in top_changes:
            report_lines.append(f"{file_id} : {change_count} changes")

        report_lines.append("\nTOP 50 FILES WITH MOST ADDITIONS")
        for file_id, add_count in top_additions:
            report_lines.append(f"{file_id} : {add_count} additions")

        if details_lines:
            report_lines.append("\nDETAILS")
            report_lines.append("=" * 80)
            report_lines.extend(details_lines)

        self.output_text.delete(1.0, tk.END)
        self.output_text.insert(tk.END, "\n".join(report_lines))
        self.progress['value'] = 100
        self.progress_label.config(text="Translation change/addition check completed!")
        self.root.update()
        ...
        if not os.path.exists(report_folder):
            os.makedirs(report_folder)
        report_path = os.path.join(report_folder, f"{tgt_name}_translation_changes_additions_report.txt")
        with open(report_path, "w", encoding="utf-8") as f:
            f.write("\n".join(report_lines))

        self.output_text.insert(tk.END, f"\n\nReport saved to: {report_path}\n")
        messagebox.showinfo("Translation Change/Additions Check",
                            f"Translation change/additions check completed.\nReport saved to:\n{report_path}")  


    def start_check_strorigin_changes(self):
        """
        GUI-entry method. Ensures exactly ONE source and ONE target folder are selected,
        then spawns a thread that runs the StrOrigin comparison.
        """
        if not self.source_folders or not self.target_folders:
            messagebox.showerror("Error", "Please select at least one source and one target folder")
            return
        ...
        if len(self.source_folders) != 1 or len(self.target_folders) != 1:
            messagebox.showerror("Error",
                                 "Please select exactly one source and one target folder for StrOrigin-change check.")
            return

        thread = threading.Thread(target=self.check_strorigin_changes_between_source_target)
        thread.start()


    def check_strorigin_changes_between_source_target(self):
        self.progress_label.config(text="Checking for StrOrigin changes and additions...")
        self.progress['value'] = 0
        self.output_text.delete(1.0, tk.END)
        self.root.update()
                ...
                for file in files:
                    ...
                    try:
                        ...
                        for element in xml_root.findall('.//LocStr'):
                            string_id = element.get('StringId', '')
                            str_origin = element.get('StrOrigin', '')
                            inner_map[string_id] = str_origin
                        file_map[file_identifier] = inner_map
                    except Exception:
                        continue
            return file_map
        ...
        for file_id in sorted(common_files):
            ...
            for key in common_keys:
                if src_dict[key] != tgt_dict[key]:
                    diffs.append({
                        'StringId': key,
                        'SourceStrOrigin': src_dict[key],
                        'TargetStrOrigin': tgt_dict[key]
                    })
                    total_change_chars += len(tgt_dict[key])
                    total_change_words += len(tgt_dict[key].split())
            ...
            if diffs:
                ...
                for d in diffs[:10]:
                    details_lines.append(f"    - StringId=\"{d['StringId']}\"")
                    details_lines.append(f"        Source StrOrigin : \"{d['SourceStrOrigin']}\"")
                    details_lines.append(f"        Target StrOrigin : \"{d['TargetStrOrigin']}\"")
                if len(diffs) > 10:
                    details_lines.append(f"    ... and {len(diffs)-10} more changes")
                details_lines.append("")
            ...
            if additions:
                ...
                for a in additions[:10]:
                    details_lines.append(f"    - StringId=\"{a['StringId']}\"")
                    details_lines.append(f"        Target StrOrigin : \"{a['TargetStrOrigin']}\"")
                if len(additions) > 10:
                    details_lines.append(f"    ... and {len(additions)-10} more additions")
                details_lines.append("")

        report_lines = []
        report_lines.append("STRORIGIN CHANGES & ADDITIONS REPORT")
        report_lines.append("=" * 80)
        report_lines.append(f"Source folder: {source_folder}")
        report_lines.append(f"Target folder: {target_folder}")
        report_lines.append(f"Generated on: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        report_lines.append("CHANGES = same StringId, different StrOrigin\nADDITIONS = new StringId with new StrOrigin value\n")

        report_lines.append("\nSUMMARY")
        report_lines.append("-" * 80)
        report_lines.append(f"Total files compared : {total_files}")
        report_lines.append(f"Total StrOrigin changes : {total_changes}")
        report_lines.append(f"Total changed characters (changes) : {total_change_chars}")
        report_lines.append(f"Total changed words (changes) : {total_change_words}")
        report_lines.append(f"Total StrOrigin additions : {total_additions}")
        report_lines.append(f"Total added characters (additions) : {total_addition_chars}")
        report_lines.append(f"Total added words (additions) : {total_addition_words}")
        report_lines.append(f"Grand total characters (changes+additions) : {total_change_chars + total_addition_chars}")
        report_lines.append(f"Grand total words (changes+additions) : {total_change_words + total_addition_words}")
        ...
        self.output_text.insert(tk.END, "\n".join(report_lines))
        self.progress['value'] = 100
        self.progress_label.config(text="StrOrigin change/addition check completed!")
        self.root.update()
        ...
        if not os.path.exists(report_folder):
            os.makedirs(report_folder)
        report_path = os.path.join(report_folder, f"{tgt_name}_strorigin_changes_additions_report.txt")
        with open(report_path, "w", encoding="utf-8") as f:
            f.write("\n".join(report_lines))

        self.output_text.insert(tk.END, f"\n\nReport saved to: {report_path}\n")
        messagebox.showinfo("StrOrigin Change/Additions Check",
                            f"StrOrigin change/additions check completed.\nReport saved to:\n{report_path}")
