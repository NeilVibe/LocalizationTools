...
def extract_rows(path: str, attr: str, regex: str, is_contained: bool):
    """
    Extract <LocStr> rows whose `attr` (either 'Str' or 'StrOrigin') matches
    the given regex.  If `is_contained` is True we keep matches, otherwise we
    keep non-matches.  Returns a pretty-printed XML string or None.
    """
    files   = get_xml_files(path)
    matches = []
