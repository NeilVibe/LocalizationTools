...
def sentence_filter(pairs: List[Tuple[str, str]]) -> List[Tuple[str, str]]:
    """
    Filter out pairs where the source is a phrase/sentence.

    Uses is_phrase() for elegant detection with acronym exception:
    - Removes entries containing '?' or '!' anywhere
    - Removes entries ending with '.' (unless they are acronyms like U.S.A.)

    Args:
        pairs: List of (source, translation) tuples

    Returns:
        Filtered list without phrase/sentence entries
    """
    return [(src, trans) for src, trans in pairs if not is_phrase(src)]
