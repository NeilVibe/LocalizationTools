...
def is_phrase(text: str) -> bool:
    ...
    Filters out text that looks like a full sentence or phrase:
    - Contains '?' or '!' anywhere
    - Ends with '.' UNLESS it's an acronym (e.g., U.S.A., A.T.A.G.)

    Acronym pattern: one or more [UppercaseLetter][Dot] sequences.
    Examples that pass (not phrases): U.S.A.  A.B.C.  NATO.
    Examples that are filtered (are phrases): "Hello." "What?" "Stop!"

    Args:
        text: The text to check

    Returns:
        True if text should be excluded as a phrase
    """
    text = text.strip()

    # Immediate filter: question or exclamation mark anywhere
    if '?' in text or '!' in text:
        return True

    # Check for trailing period
    if text.endswith('.'):
        # Exception: acronyms like U.S.A. or A.B.C. (UpperLetter+Dot pattern)
        if re.fullmatch(r"([A-Z]\.)+", text):
            return False  # It's an acronym — not a phrase
        return True  # Trailing period = phrase
