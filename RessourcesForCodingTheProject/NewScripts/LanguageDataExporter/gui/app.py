...
from exporter.gamedata_index import GameDataReverseIndex
from exporter.glossary_extractor import extract_and_write_glossary
...
class LanguageDataExporterGUI:
    ...
    def _update_path_labels(self):
        ...
        self.gamedata_path_label.config(text=str(config.STATICINFO_FOLDER))
        self._check_paths()
    ...
    def _build_path_info_section(self):
        ...
        # GameData StaticInfo folders (auto-derived, used for glossary)
        ttk.Label(section, text="StaticInfo:").grid(row=3, column=0, sticky="w", padx=5)
        self.gamedata_path_label = ttk.Label(
            section,
            text=str(config.STATICINFO_FOLDER),
            foreground="blue",
        )
        self.gamedata_path_label.grid(row=3, column=1, sticky="w", padx=5)
        self.gamedata_status = ttk.Label(section, text="")
        self.gamedata_status.grid(row=3, column=2, sticky="w", padx=5)

        # Output Folder
        ttk.Label(section, text="Output Folder:").grid(row=4, column=0, sticky="w", padx=5, pady=5)
    ...
    def _check_paths(self):
        ...
        # GameData: show which subfolders are available
        gd_found = []
        if config.STATICINFO_FOLDER.exists():
            gd_found.append("StaticInfo")
        if config.STATICINFO_QUEST_FOLDER.exists():
            gd_found.append("Quest")
        if config.STATICINFO_DIALOG_FOLDER.exists():
            gd_found.append("Dialog")
        if gd_found:
            self.gamedata_status.config(
                text=f"OK ({', '.join(gd_found)})",
                foreground="green",
            )
        else:
            self.gamedata_status.config(
                text="NOT FOUND (glossary disabled)",
                foreground="orange",
            )
    ...
    def _build_actions_section(self):
        ...
        # Row 2: Separator + Glossary button (visually distinct)
        ttk.Separator(section, orient="horizontal").pack(fill="x", pady=4)

        gloss_frame = ttk.Frame(section)
        gloss_frame.pack(fill="x")

        self.btn_glossary = ttk.Button(
            gloss_frame,
            text="Extract Glossary (GameData)",
            command=self._extract_glossary,
            width=28
        )
        self.btn_glossary.pack(side="left", padx=10, ipady=8)

        ttk.Label(
            gloss_frame,
            text="Extracts Name-type terms from GameData — auto-uses same branch/drive",
            font=("Arial", 8),
            foreground="gray",
    ...
    def _set_buttons_enabled(self, enabled: bool):
        ...
        self.btn_glossary.config(state=state)
    ...
    def _extract_glossary(self):
        """Extract glossary using GameData reverse index."""
        loc_folder = config.LOC_FOLDER
        ...
        # Check if at least one GameData folder exists
        gamedata_folders = [
            config.STATICINFO_FOLDER,
            config.STATICINFO_QUEST_FOLDER,
            config.STATICINFO_DIALOG_FOLDER,
        ]
        available_folders = [f for f in gamedata_folders if f.exists()]
        if not available_folders:
            messagebox.showerror(
                "Error",
                "No GameData folders found. Need at least one of:\n"
                f"  - {config.STATICINFO_FOLDER}\n"
                f"  - {config.STATICINFO_QUEST_FOLDER}\n"
                f"  - {config.STATICINFO_DIALOG_FOLDER}\n\n"
                "Make sure the drive and branch are correct."
            )
            return
        ...
        self._set_status("Building GameData reverse index...")
        self.progress["value"] = 0
        self._set_buttons_enabled(False)
        ...
        def worker():
            try:
                ...
                reverse_index.build(available_folders, progress_callback=progress_cb)

                stats = reverse_index.stats
                progress_cb(
                    f"GameData indexed: {stats['total_korean_texts']} Korean texts "
                    f"({stats['name_entries']} Name, {stats['desc_entries']} Desc) "
                    f"from {stats['files_parsed']} files"
                )

                self.root.after(0, lambda: self._update_progress(30))

                # Step 2: Extract glossary for each language
                glossary_output = OUTPUT_FOLDER / "Glossary"

                results = extract_and_write_glossary(
                    reverse_index=reverse_index,
                    loc_folder=loc_folder,
                    output_folder=glossary_output,
                ...
                lang_summary = ", ".join(
                    f"{lang.upper()}: {count}" for lang, count in sorted(results.items())
                )

                self.root.after(0, lambda: self._set_status(
                    f"Glossary extracted: {total_terms} terms across {len(results)} languages"
                ))
                self.root.after(0, lambda: self._set_buttons_enabled(True))
                self.root.after(0, lambda: messagebox.showinfo(
                    "Glossary Extraction Complete",
                    f"Extracted glossary for {len(results)} languages:\n\n"
                    f"{lang_summary}\n\n"
                    f"Output folder:\n{glossary_output}"
                ))
                ...
                self.root.after(0, lambda err=str(ex): messagebox.showerror(
                    "Error", f"Glossary extraction failed: {err}"
                ))
                self.root.after(0, lambda: self._set_status("Glossary extraction failed"))
                self.root.after(0, lambda: self._set_buttons_enabled(True))
