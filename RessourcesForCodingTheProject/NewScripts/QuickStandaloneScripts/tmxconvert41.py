...
def clean_xml_folder_data(input_folder):
    """
    Cleans all .xml files in the input_folder recursively.
    Uses Group 1/Group 2 regex technique to target ONLY the 'Str' attribute value.
    - Group 1: Str="
    - Group 2: The attribute content
    - Converts NBSP (\u00A0) to normal spaces.
    - Removes zero-width characters.
    - Normalizes all line break variants to &lt;br/&gt;.
    ...
    # Pattern: Group 1 captures (Str="), Group 2 captures (content)
    ...
    def process_attribute_value(match):
        group1 = match.group(1)  # Str="
        val = match.group(2)     # The content inside the quotes

        # 1. Convert NBSP (\u00A0) to normal space inside Str attribute
        val = val.replace('\u00A0', ' ')
        
        # 2. Remove Zero-Width characters
        val = zero_width_pattern.sub('', val)
        
        # 3. Normalize line breaks and BR tags to &lt;br/&gt;
        ...
        # 4. Clean up &desc; markers
        val = val.replace('&amp;desc;', '').replace('&desc;', '')
        
        # Return Group 1 + Processed Group 2 + closing quote
        return f'{group1}{val}"'
    ...
    for dirpath, _, filenames in os.walk(input_folder):
        for fname in filenames:
            ...
            try:
                ...
                # Regex transformation targeting only the Str="..." content
                cleaned_content = str_attr_pattern.sub(process_attribute_value, raw_content)

                # Use lxml to normalize Source text (StrOrigin) and ensure valid XML
                parser = etree.XMLParser(recover=True, remove_blank_text=False)
                root = etree.fromstring(cleaned_content.encode("utf-8"), parser=parser)

                for loc in root.iter("LocStr"):
                    # Normalize Source text (StrOrigin) for consistency
                ...
                print(f"[CLEAN] Processed (NBSP & Breaks fixed in Str): {fpath}")

            except Exception as e:
                print(f"[CLEAN] Error cleaning {fpath}: {e}")

    print("[CLEAN] XML folder cleaning completed.")
