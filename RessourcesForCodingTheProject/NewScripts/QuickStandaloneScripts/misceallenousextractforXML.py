...
def extract_rows_dual(path: str,
                      regex_str: str, is_contained_str: bool,
                      regex_so: str,  is_contained_so: bool):
    """
    Extract <LocStr> rows that satisfy BOTH conditions at the same time:
      - Condition A applies to Str
      - Condition B applies to StrOrigin

    Each condition can be "contained" or "NOT contained".
    Empty regex means "no condition" for that field.
    """
    files   = get_xml_files(path)
    matches = []

    pat_str = re.compile(regex_str, re.IGNORECASE) if regex_str else None
    pat_so  = re.compile(regex_so,  re.IGNORECASE) if regex_so  else None
    ...
    for f in files:
        ...
        for loc in root.iter("LocStr"):
            ...
            if pat_str is None:
                ...
                found_str = bool(pat_str.search(val_str))
                ok_str = (found_str if is_contained_str else (not found_str))

            # StrOrigin condition
            if pat_so is None:
                ok_so = True
            else:
                found_so = bool(pat_so.search(val_so))
                ok_so = (found_so if is_contained_so else (not found_so))
...
class SimpleXMLTool(tk.Tk):
    def __init__(self):
        ...
        # Extract (dual: Str + StrOrigin AND)
        self.regex_str       = tk.StringVar()
        self.regex_so        = tk.StringVar()
        self.is_contained_str = tk.BooleanVar(value=True)
        self.is_contained_so  = tk.BooleanVar(value=True)
    ...
    def create_widgets(self):
        ...
        # Extract (single)
        frm_extract = tk.LabelFrame(self, text="3. Extract Rows (Single Condition)")
        ...
        # Extract (dual AND)
        frm_extract2 = tk.LabelFrame(self, text="3b. Extract Rows (Str AND StrOrigin)")
        frm_extract2.pack(fill="x", padx=10, pady=5)

        tk.Label(frm_extract2, text="Str Regex:").grid(row=0, column=0, sticky="w")
        tk.Entry(frm_extract2, textvariable=self.regex_str, width=30)\
            .grid(row=0, column=1, sticky="w")
        ttk.Radiobutton(frm_extract2, text="contained",
                        variable=self.is_contained_str, value=True)\
            .grid(row=0, column=2, padx=5)
        ttk.Radiobutton(frm_extract2, text="NOT contained",
                        variable=self.is_contained_str, value=False)\
            .grid(row=0, column=3, padx=5)

        tk.Label(frm_extract2, text="StrOrigin Regex:").grid(row=1, column=0, sticky="w")
        tk.Entry(frm_extract2, textvariable=self.regex_so, width=30)\
            .grid(row=1, column=1, sticky="w")
        ttk.Radiobutton(frm_extract2, text="contained",
                        variable=self.is_contained_so, value=True)\
            .grid(row=1, column=2, padx=5)
        ttk.Radiobutton(frm_extract2, text="NOT contained",
                        variable=self.is_contained_so, value=False)\
            .grid(row=1, column=3, padx=5)

        tk.Button(frm_extract2, text="Extract (AND)", command=self.do_extract_dual,
                  bg="#cfe8ff")\
            .grid(row=2, column=0, columnspan=4, pady=5, sticky="ew")
    ...
    def save_extracted_xml(self, xml_str: str):
        ...
        if save_path:
            with open(save_path, "w", encoding="utf-8") as f:
                f.write(xml_str)
            messagebox.showinfo("Saved", f"Extracted XML saved to {save_path}")
    ...
    def do_extract(self):
        ...
        if not xml_str:
            ...
            return
        self.save_extracted_xml(xml_str)

    def do_extract_dual(self):
        path = self.select_path()
        if not path:
            return

        regex_str = self.regex_str.get().strip()
        regex_so  = self.regex_so.get().strip()

        if not regex_str and not regex_so:
            messagebox.showerror("Error", "Enter at least one regex (Str or StrOrigin).")
            return

        xml_str = extract_rows_dual(
            path,
            regex_str, self.is_contained_str.get(),
            regex_so,  self.is_contained_so.get()
        )
        if not xml_str:
            messagebox.showinfo("No Matches", "No matching rows found.")
            return
