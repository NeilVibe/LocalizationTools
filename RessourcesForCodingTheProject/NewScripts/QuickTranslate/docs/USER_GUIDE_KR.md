...
Excel/XML의 번역 수정 사항을 `languagedata_*.xml` 파일에 기록하는 도구입니다. 세 개의 탭으로 구성:

| 탭 | 내용 |
|----|------|
| **Transfer** | 매치 타입, Source/Target 경로, TRANSFER 버튼, 사전 제출 검사, 설정 |
| **Other Tools** | Find Missing Translations, 제외 규칙 |
| **TMX Tools** | MemoQ-TMX 변환, TMX 클리너 → Excel 내보내기 |
...
- **텍스트 무결성 문제** → 해당 행 차단 (깨진 `<br/>`, 단독 `<` 또는 `>`, 대체 문자, 제어 문자)
...
- 텍스트 무결성 (깨진 `<br/>`, 단독 `<` 또는 `>`, U+FFFD 대체 문자, 제어 문자 → 행 건너뜀)
...
## 6. TMX Tools (TMX Tools 탭)

### MemoQ-TMX 변환

languagedata XML/Excel 파일을 MemoQ 호환 TMX 파일로 변환합니다. 폴더/파일 접미사로 언어를 자동 감지합니다.

**사용 방법:**

1. **Folder** 또는 **Single File** 모드 선택
2. 수정 파일 폴더 또는 파일 찾아보기
3. 상태 레이블에 감지된 언어 표시 (예: "Detected: FRE (3 files), GER (2 files)")
4. **Convert to MemoQ-TMX** 클릭

**처리 내용:**

- 언어 접미사가 있는 파일/폴더 스캔 (Transfer 탭과 동일한 감지 방식)
- 언어별 `.tmx` 파일 생성 (예: `MyFolder_FRE.tmx`)
- KOR 건너뜀 (소스 언어 — 빈 TMX 생성됨)
- (StrOrigin, StringID) 기준 중복 제거, 최신 파일 버전 유지
- MemoQ `bpt/ept/ph` 마크업 추가 (StaticInfo 코드, `{code}` 패턴, `<br/>`)
- Desc TU (음성 연출 설명) 포함 (있을 경우)

**로그 출력:** 언어 감지, 파일별 진행률, TU 수, 중복 제거 통계, 한국어 건너뛴 수, 후처리 상태, 최종 요약.

### TMX 클리너 → Excel

TMX 파일에서 모든 CAT 도구 마크업을 제거하고 병합 가능한 Excel로 내보냅니다.

**사용 방법:**

1. **Select TMX → Clean & Export to Excel** 클릭
2. `.tmx` 파일 선택
3. 출력: 같은 폴더에 `파일명_clean.xlsx`

**정리 대상:**

- MemoQ `bpt/ept` StaticInfo 쌍 → `{StaticInfo:Category:ID#text}`
- 일반 `bpt/ept` 서식 태그 (Trados 굵게/기울임) → 내부 텍스트 유지
- `ph` 플레이스홀더 → 5단계 우선순위 스마트 추출 (val → 서식 → displaytext → 구조 → 원본)
- `it`, `x`, `g` 태그 → 제거 또는 내부 텍스트 유지
- 제로 너비 문자 제거
- 모든 `<br>` 변형 → `<br/>`로 정규화

**Excel 출력:** 3열 (StrOrigin, Correction, StringID) — Transfer 소스로 바로 사용 가능.

**로그 출력:** `[1/3]` 파싱 + 정리, `[2/3]` 중복 제거 통계, `[3/3]` Excel 작성, TU 수 및 대상 언어 감지 포함.
...
**TMX Tools 탭:** Convert to MemoQ-TMX, Select TMX → Clean & Export to Excel
...
| **TMX** | Translation Memory eXchange — 번역 메모리 표준 형식 |
| **MemoQ-TMX** | MemoQ 전용 `bpt/ept/ph` 마크업이 포함된 TMX (StaticInfo 코드, 플레이스홀더) |
| **TU** | Translation Unit — TMX 파일의 원본-번역 쌍 1건 |
| **CAT 도구** | 컴퓨터 보조 번역 도구 (MemoQ, Trados 등) |
