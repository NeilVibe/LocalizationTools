...
, <br/>, &#10;) to &lt;br/&gt;.
    ...
    def process_attribute_value(match):
        prefix = match.group(1)  # Str="
        val = match.group(2)     # The actual content

        # 1. Convert NBSP to normal space
        val = val.replace('\u00A0', ' ')
        ...
        # 4. Clean up &desc; markers inside the string
    ...
    for dirpath, _, filenames in os.walk(input_folder):
        for fname in filenames:
            ...
            try:
                ...
                # Perform Regex cleaning on the raw text targeting the Str attribute
                # This ensures we only touch the translatable content
                cleaned_content = str_attr_pattern.sub(process_attribute_value, raw_content)

                # Global clean up for any remaining &desc; outside attributes if necessary
                cleaned_content = cleaned_content.replace('&desc;', '&amp;desc;')

                # Parse to ensure valid XML structure and handle the actual LocStr objects
                parser = etree.XMLParser(recover=True, remove_blank_text=False)
                root = etree.fromstring(cleaned_content.encode("utf-8"), parser=parser)

                # Final pass through lxml to ensure StrOrigin or other attributes are also healthy
                # (Line break normalization for Str is already handled by the regex sub)
                for loc in root.iter("LocStr"):
