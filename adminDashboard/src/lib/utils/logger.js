 ...
 * Comprehensive logging with console, file, and backend integration
...
const config = {
  ...
  sendToBackend: true,
  maxLogMessages: 1000,
  logFilePath: 'logs/dashboard_app.log',
  errorLogFilePath: 'logs/dashboard_error.log'
};

// Lazy-loaded API client (avoid circular dependency)
let apiClient = null;
...
/**
 * Send log to backend server (fire and forget)
 */
async function sendToBackend(level, message, data) {
  if (!config.sendToBackend) return;
  if (level !== LOG_LEVELS.ERROR && level !== LOG_LEVELS.CRITICAL) return;
...
logger.info('Dashboard logger initialized', {
  ...
  sendToBackend: config.sendToBackend
});
