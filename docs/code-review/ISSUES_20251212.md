...
### [ ] DR1-001: get_table_counts() Incomplete
**Priority:** MEDIUM | **File:** `server/database/db_setup.py:154-185`
**Issue:** Only counts 12 tables, missing 15+ tables (LDM, Telemetry).
**Fix:** Add all tables or make dynamic using `Base.metadata.tables`.

### [ ] DR1-002: chunked_query() No ORDER BY
**Priority:** MEDIUM | **File:** `server/database/db_utils.py:637-674`
**Issue:** OFFSET/LIMIT without ORDER BY = undefined behavior in SQL.
**Fix:** Default to primary key if order_by not specified.

### [ ] DR1-003: upsert_batch() Return Incorrect
**Priority:** MEDIUM | **File:** `server/database/db_utils.py:676-719`
**Issue:** Always returns `updated=0`, never counts actual updates.
**Fix:** Simplify to `{'total': N}` or use RETURNING clause.

### [ ] DR1-004: LDMTrash.item_data Wrong Type
**Priority:** MEDIUM | **File:** `server/database/models.py:899`
**Issue:** Uses `Text` for JSON data instead of `JSONB`.
**Fix:** Change to `JSONB` type.

### [ ] DR1-005: JSON Columns Should Be JSONB
**Priority:** MEDIUM | **File:** `server/database/models.py` (9 columns)
**Issue:** `JSON` stores as text, `JSONB` is binary (faster, indexable).
**Affected:** LogEntry, ActiveOperation, UserActivitySummary, Installation, RemoteLog, TelemetrySummary
**Fix:** Migrate all `JSON` to `JSONB`.
...
### [ ] DR2-001: Async Pool Settings Hardcoded
...
**Fix:** Use `config.DB_POOL_SIZE` and `config.DB_MAX_OVERFLOW` like sync engine.

### [ ] DR2-002: Auto-Commit in get_async_db
...
**Fix:** Document behavior or let caller control commit explicitly.

### [ ] DR2-003: MD5 Hash for Cache Keys
...
**Fix:** Add comment: "MD5 used for cache key uniqueness, not security".

### [ ] DR2-004: Deprecated asyncio.get_event_loop().time()
**Priority:** MEDIUM | **File:** `server/utils/websocket.py:62`
**Issue:** `asyncio.get_event_loop().time()` deprecated in Python 3.10+.
**Fix:** Use `time.time()` or `datetime.utcnow().timestamp()`.

### [ ] DR2-005: file_info/parameters Stored as str()
...
### [ ] DR3-002: Duplicate Sync/Async Auth Endpoints
...
**Fix:** Deprecate sync version or document which to use.

### [ ] DR3-003: Sync Auth Missing Audit Logging
**Priority:** MEDIUM | **File:** `server/api/auth.py`
**Issue:** Doesn't use `log_login_success`, `log_login_failure` - no audit trail.
**Fix:** Add audit logging like async version, or deprecate sync version.

### [ ] DR3-004: list_users Without ORDER BY
**Priority:** MEDIUM | **File:** `server/api/auth.py:176`
**Issue:** `db.query(User).offset(skip).limit(limit)` - pagination without order.
**Fix:** Add `.order_by(User.user_id)` for consistent pagination.

### [ ] DR3-005: No Rate Limiting on Login
**Priority:** MEDIUM | **File:** `server/api/auth.py`, `server/api/auth_async.py`
**Issue:** Login endpoints have no rate limiting. Brute-force possible.
**Fix:** Add rate limiter middleware (slowapi) or use audit_logger's `get_failed_login_count()`.

### [ ] DR3-006: Hardcoded Role Validation
**Priority:** MEDIUM | **File:** `server/api/auth_async.py:422, 496`
**Issue:** `valid_roles = ["user", "admin"]` duplicated twice.
**Fix:** Create `VALID_ROLES` constant or use enum from models.
...
### [ ] DR3-009: DEV_MODE for Claude Autonomous Testing
**Priority:** MEDIUM | **File:** `server/config.py`, `server/utils/dependencies.py`, `server/api/auth_async.py`
**Issue:** No way for Claude to work autonomously without manual login. Commenting out auth is dangerous.
**Fix:** Implement secure DEV_MODE with:
...
**Priority:** MEDIUM | **File:** `server/tools/ldm/api.py:867`
**Issue:** `next(get_db())` creates sync session inside async function. Blocks event loop.
**Fix:** Use async session or wrap in `run_in_executor()`.

### [ ] DR4-002: Repeated Sync DB Pattern in TM Search
**Priority:** MEDIUM | **File:** `server/tools/ldm/api.py:969, 1003, 1028`
**Issue:** Multiple endpoints use `sync_db = next(get_db())` pattern.
**Fix:** Create async versions of TMManager methods.
...
**Fix:** Implement or remove. Don't ship TODO stubs.
...
### [ ] DR5-001: excel_column_to_index Only Works for A-Z
...
**Fix:** Support multi-letter columns (AA, AB, etc.):
```python
...
### [ ] DR5-002: Model Save Without Permission Check
...
**Fix:** Add try/except and check write permission before save.
...
**Fix:** Use centralized `text_utils.normalize_text()` - was created in Quick Scan for this.
...
### [ ] DR6-001: Missing `import re` in parser.py
...
### [ ] DR6-002: etree Used Without HAS_LXML Guard
...
**Fix:** Add `if not HAS_LXML: raise ImportError("lxml required")` before XML operations.
...
**Priority:** MEDIUM | **File:** `server/api/stats.py` (multiple lines)
...
**Severity:** Lower than DR3-001 (read-only stats vs user data), but still a gap.
**Fix:** Implement DR3-009 (DEV_MODE flag) to handle this properly.

### [~] DR8-002: base_tool_api Creates New Engine Per Sync Call
**Priority:** LOW | **File:** `server/api/base_tool_api.py:256, 321`
**Accept:** `create_engine()` called in sync methods. Works but creates new connections. Background task use case makes this acceptable.
...
### [ ] DR10-001: Hardcoded API_BASE in VirtualGrid
...
**Fix:** Import and use `serverUrl` from stores like other components.

### [ ] DR10-002: Hardcoded API_BASE in DataGrid
**Priority:** MEDIUM | **File:** `locaNext/src/lib/components/ldm/DataGrid.svelte:25`
**Issue:** Same hardcoded URL pattern.
**Fix:** Use serverUrl from stores.
...
| Session 8 | 3 | 1 | 0 | 2 | 0 |
| Session 9 | 2 | 0 | 0 | 2 | 0 |
| Session 10 | 3 | 2 | 0 | 1 | 0 |
| Session 11 | 2 | 1 | 0 | 1 | 0 |
| Session 12 | 2 | 0 | 0 | 2 | 0 |
| **TOTAL** | **66** | **31** | **5** | **29** | **1** |

**✅ HIGH PRIORITY FIXED:** DR3-001 (Admin check re-enabled)
**📋 FEATURE REQUEST:** DR3-009 (DEV_MODE flag for autonomous testing)
