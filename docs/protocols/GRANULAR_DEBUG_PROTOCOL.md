...
NEVER FILTER LOGS ────────────────────────► READ FULL CONTEXT (NO GREP!)
```

**NUMBER YOUR GDP MARKERS** - Creates traceable execution path:
```javascript
...
| Platform | Jump To |
|----------|---------|
| Python/FastAPI | [Python Debugging](#python-fastapi-debugging) |
| Node.js/Electron | [Node.js Debugging](#nodejs-electron-debugging) |
| Svelte/Frontend | [Frontend UI Debugging](#frontend-ui-debugging-svelte-components) |
| Windows App | [Windows App Debugging](#windows-packaged-app-debugging) |
...
## GDP Analysis Output Format

When documenting a bug investigation, use this format:
...
npx playwright test --trace on        # Generate trace on failure
npx playwright test tests/specific.spec.js  # Run single test
```
...
| Issue | Symptom | Fix |
|-------|---------|-----|
| Missing `{#each}` key | UI flickers | Add `(item.id)` key |
| Svelte 4 `$:` syntax | Not reactive | Use `$derived()` / `$effect()` |
| State not reactive | UI stale | Use `$state()` |
| Array mutation not detected | UI not updating | Svelte 5 proxies work, check logic |
...
```bash
# From WSL - App main log (next to exe)
...
const logFile = path.join(exeDir, 'logs', 'debug.log');
...
*Case studies: TM Paste Bug, Patch Updater, Python Partial Installs*
...
*Protocol established: 2026-01-11*
*Updated: 2026-02-01 - Added platform-specific sections, GDP output format, consolidated with agent*
