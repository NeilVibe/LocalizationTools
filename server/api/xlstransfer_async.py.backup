        ...
        try:
            ...
            if result.get("success"):
                ...
                if result.get("output_dir"):
                    operation.output_dir = result["output_dir"]
                    logger.info(f"Stored output directory: {operation.output_dir}")
                ...
                asyncio.run(emit_operation_complete({
                    ...
                    'started_at': operation.started_at.isoformat(),
                    'completed_at': operation.completed_at.isoformat(),
                    'output_dir': operation.output_dir  # Include output directory for frontend download
                }))
            else:
                operation.status = "failed"
