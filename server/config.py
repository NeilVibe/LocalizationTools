...
# TLS/SSL -- reuse certs generated by setup wizard for PostgreSQL
# If server.crt + server.key exist in DATA_DIR, uvicorn runs HTTPS.
# If not (dev mode, old installs), runs plain HTTP. Fully backward compatible.
SSL_CERT_FILE = DATA_DIR / "server.crt"
SSL_KEY_FILE = DATA_DIR / "server.key"
SSL_ENABLED = SSL_CERT_FILE.exists() and SSL_KEY_FILE.exists()
...
if __name__ == "__main__":
    ...
    import re as _re
    _safe_url = _re.sub(r'://([^:]+):(.+)@(?=[^@]*$)', r'://\1:***@', str(DATABASE_URL))
    print(f"\nDatabase URL: {_safe_url}")
