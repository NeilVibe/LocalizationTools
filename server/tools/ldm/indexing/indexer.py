...
class TMIndexer:
        ...
        for entry in entries:
            ...
            if normalized not in lookup:
                if entry.get("string_id"):
                    lookup[normalized] = {"variations": [entry_data], "source_text": source}
                else:
                ...
                if "variations" in existing:
                    ...
                    lookup[normalized] = {"variations": [existing, entry_data], "source_text": source}

            stripped = normalized.strip()
            if stripped and stripped != normalized and stripped not in lookup:
                if entry.get("string_id"):
                    lookup[stripped] = {"variations": [entry_data], "source_text": source}
                else:
    ...
    def _build_whole_embeddings(self, entries: List[Dict], tm_path: Path, batch_size: int = 64) -> Dict[str, Any]:
        ...
        FAISSManager.build_index(embeddings, path=tm_path / "faiss" / "whole.index", normalize=True)
    ...
    def _build_line_embeddings(self, entries: List[Dict], tm_path: Path, batch_size: int = 64) -> Dict[str, Any]:
        ...
        FAISSManager.build_index(embeddings, path=tm_path / "faiss" / "line.index", normalize=True)
    ...
    def load_indexes(self, tm_id: int) -> Dict[str, Any]:
        ...
        line_index = FAISSManager.load_index(line_index_path) if line_index_path.exists() else None
