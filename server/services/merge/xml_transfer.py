# Origin: QuickTranslate/core/xml_transfer.py
    ...
    def _try_resolve_as_eventname(eventname_str):
        ...
        if not _en_resolver_loaded:
            ...
            try:
                ...
                _en_mapping = get_eventname_mapping(get_config().EXPORT_FOLDER)
...
def transfer_folder_to_folder(
    ...
    Args:
        ...
        threshold: Similarity threshold for fuzzy modes (defaults to get_config().FUZZY_THRESHOLD_DEFAULT)
    ...
    effective_threshold = threshold if threshold is not None else get_config().FUZZY_THRESHOLD_DEFAULT
        ...
        try:
            if source_file.suffix.lower() == ".xml":
                ...
                if formula_report:
                    ...
                    msg = (
                        f"WARNING: {formula_count} entry(ies) in {source_file.name} "
    ...
    if all_have_eventnames:
        ...
        en_mapping = get_eventname_mapping(get_config().EXPORT_FOLDER, progress_callback=progress_callback)
    ...
    def _get_recovery_mapping():
        ...
        if _recovery_mapping is None:
            ...
            _recovery_mapping = get_eventname_mapping(get_config().EXPORT_FOLDER)
    ...
    if all_missing_eventnames:
        ...
        report_dir = get_config().get_failed_report_dir(
