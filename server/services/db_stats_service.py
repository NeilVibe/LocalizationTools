"""
DB Stats Service - Database performance monitoring and health checks.

Extracts business logic from admin_db_stats routes.
Uses sync Session (not AsyncSession) because the underlying routes use sync DB.
...
class DbStatsService:
    """Service layer for database performance statistics and health checks."""
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        Returns connection pool status, query stats, table sizes, and configuration.
        """
        stats = {
        ...
        try:
            ...
            if dialect == "postgresql":
                stats.update(self._get_postgresql_stats())
            else:
    ...
    def get_health(self) -> Dict[str, Any]:
        ...
        Returns status (healthy/warning/critical), issues, and recommendations.
        """
        health = {
            "status": "healthy",
            "timestamp": datetime.utcnow().isoformat(),
        ...
        try:
            ...
            """)).scalar()
            max_conn = int(self.db.execute(text("SHOW max_connections")).scalar())
            ...
            bloat_result = self.db.execute(text("""
    ...
    def _get_postgresql_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Max connections
            max_conn = self.db.execute(text("SHOW max_connections")).scalar()
            ...
            for param in config_params:
                try:
                    value = self.db.execute(text(f"SHOW {param}")).scalar()
