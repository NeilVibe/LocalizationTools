...
import os, sqlite3
try:
    conn = sqlite3.connect(os.path.expanduser('~/LocalizationTools/server/data/localizationtools.db'))
    c = conn.cursor()
...
PYEOF
    ...
    # Recent log errors
    ERROR_COUNT=$(tail -50 $HOME/LocalizationTools/server_output.log 2>/dev/null | grep -i "ERROR" | wc -l)
    if [ "$ERROR_COUNT" -eq 0 ]; then
        echo -e "${GREEN}✓ Logs: No recent errors${NC}"
    else
        echo -e "${YELLOW}⚠ Logs: $ERROR_COUNT error(s) in last 50 lines${NC}"
        echo "  Last error:"
        tail -50 $HOME/LocalizationTools/server_output.log 2>/dev/null | grep -i "ERROR" | tail -1 | sed 's/^/  /' | cut -c1-80
    fi
    echo
